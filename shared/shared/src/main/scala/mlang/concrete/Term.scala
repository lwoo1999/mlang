...
object Declaration {
  ...
  case class Define(modifiers: Seq[Modifier], form: InvocationForm, parameters: Seq[NameType], typ: Option[Term], term: Term) extends Declaration
  // depending on our algorithm, recursive ones might not need to declare first
  case class Declare(modifiers: Seq[Modifier], form: InvocationForm, parameters: Seq[NameType], typ: Term) extends Declaration
}
